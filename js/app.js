...
window.onload = function () {
  const confirmacao = confirm(
    "Você deseja enviar email para os pais dos alunos com menos de 80% de presença?"
  );
...
const loadPresences = async () => {
  ...
  try {
    const alunosSnapshot = await db
      ...
      .where("turma", "==", turmaId)
      .get();
    const presencasSnapshot = await db
      .collection("Presenças")
      .where("turma", "==", turmaId)
      .where("date", "==", date)
      .get();
...
document
  .getElementById("load-presences")
  .addEventListener("click", loadPresences);
...
const registerPresence = (event) => {
  ...
  if (!turmaId || !date) {
    console.error(
      "Erro ao registrar presença: Turma ou data não especificada."
    );
    return;
  }
...
document
  .getElementById("register-view-presence-form")
  .addEventListener("submit", registerPresence);
